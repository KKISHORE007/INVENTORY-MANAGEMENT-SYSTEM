...
const SalesOrders = () => {
  ...
  const addItemRow = () => {
    setFormData({
      ...formData,
      items: [...formData.items, { product: '', quantity: 1, sellingPrice: 0 }]
    });
  };
  ...
  const updateItem = (index, field, value) => {
    ...
    // Auto-fill selling price if product is selected
    if (field === 'product') {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Sales Order created successfully (Draft mode)');
      setIsModalOpen(false);
      setFormData({ customer: '', warehouse: '', notes: '', items: [] });
      fetchSOs();
    } catch (err) {
      toast.error(err.response?.data?.message || 'Error creating SO');
    }
  };

  const handleConfirm = async (soId) => {
    if (window.confirm('Confirm this order? This will instantly deduct stock from the warehouse.')) {
      try {
        await api.post(`/sales-orders/${soId}/confirm`);
        toast.success('Sales Order Confirmed and Stock Deducted');
        fetchSOs();
      } catch (err) {
        toast.error(err.response?.data?.message || 'Error confirming SO');
      }
    }
  };
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {sos.map((so) => (
                <tr key={so._id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900">{so.soNumber}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{so.customer?.name}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                      so.status === 'Confirmed' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                    }`}>
                      {so.status}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    {so.status === 'Draft' && (
                      <button onClick={() => handleConfirm(so._id)} className="text-blue-600 hover:text-blue-900 flex items-center justify-end w-full">
                        <CheckCircle className="w-4 h-4 mr-1" /> Confirm Order
                      </button>
                    )}
                    {so.status === 'Confirmed' && (
                      <span className="text-green-600 flex items-center justify-end w-full"><Truck className="w-4 h-4 mr-1" /> Ready to Ship</span>
                    )}
                  </td>
                </tr>
              ))}
              {sos.length === 0 && (
                <tr><td colSpan="5" className="px-6 py-4 text-center text-gray-500">No Sales Orders found.</td></tr>
              )}
            </tbody>
          </table>
        )}
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-3xl shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Fulfill from Warehouse</label>
              ...
              <div className="border border-gray-200 rounded-md p-4 bg-gray-50">
                ...
                {formData.items.map((item, index) => (
                  <div key={index} className="flex gap-2 mb-2 items-end">
                    ...
                    <div className="w-32">
                      <input type="number" step="0.01" required placeholder="Price" value={item.sellingPrice} onChange={(e) => updateItem(index, 'sellingPrice', Number(e.target.value))} className="w-full px-2 py-1 text-sm border border-gray-300 rounded" />
                    </div>
              ...
              <div className="flex justify-end space-x-3 pt-4 border-t border-gray-100 mt-4">
                ...
                <button type="submit" className="px-4 py-2 bg-primary text-white rounded-md hover:bg-blue-600">Save Draft Order</button>
              </div>
            </form>
          </div>
        </div>
